...
def write_emodb_csv(emotions=["sad", "neutral", "happy"], train_name="train_emo.csv",
    ...
    for emotion, code in categories_reversed.items():
        if emotion not in emotions:
            del categories[code]
    for file in glob.glob(filename + "data/emodb/wav/*.wav"):
        try:
...
def write_tess_ravdess_csv(emotions=["sad", "neutral", "happy"], train_name="train_tess_ravdess.csv",
    ...
    for category in emotions:
        ...
        for i, path in enumerate(glob.glob(f"{filename}data/training/Actor_*/*_{category}.wav")):
        ...
        for i, path in enumerate(glob.glob(f"{filename}data/validation/Actor_*/*_{category}.wav")):
...
def write_custom_csv(emotions=['sad', 'neutral', 'happy'], train_name="train_custom.csv", test_name="test_custom.csv",
                    verbose=1):
    """
    Reads Custom Audio data fromfilename +  data/*-custom and then writes description files (csv)
    params:
        emotions (list): list of emotions to read from the folder, default is ['sad', 'neutral', 'happy']
    ...
    for category in emotions:
        # train data
        for i, file in enumerate(glob.glob(f"{filename}data/train-custom/*_{category}.wav")):
        ...
        # test data
        for i, file in enumerate(glob.glob(f"{filename}data/test-custom/*_{category}.wav")):
